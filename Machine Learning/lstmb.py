...
def LSTM_Bidir_model(word_index, embedding_matrix, max_len, categorical):
    ...
    model = Sequential()
    model.add(Embedding(len(word_index)+1, 100, weights=[embedding_matrix], input_length=max_len, trainable=True))
    model.add(Dropout(0.4)) #0.0
    model.add(Conv1D(filters=32, kernel_size=3, padding='same', activation='relu'))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Bidirectional(LSTM(128), merge_mode='concat', weights=None))    
    if (categorical):
...
def LSTM_Bidir(X_train, y_train, X_test, y_test, word_index, embedding_matrix, max_len, seed, categorical):
    ...
    for i,(train, test) in enumerate(kfold):
        ...
        #plot_model_history(model_info) ADD BACK IN FOR PLOTTING
...
#f = open('ML_picklefiles/LSTMB_binary_pre.pckl', 'wb') 
#pickle.dump(avg_metrics, f)
#f.close()

# Categorical Tests
y_pred, metrics, model_infos = LSTM_Bidir(X_train_c, y_train_c, X_test_c, y_test_c, word_index, embedding_matrix_w2v, max_len, seed, True)
avg_metrics = findAverage(metrics)
print("Average Scores")
print(avg_metrics)
...
#create picklefile of categorical results untrained
#f = open('ML_picklefiles/LSTMB_categorical_post.pckl', 'wb') 
#pickle.dump(avg_metrics, f)
#f.close()

#create picklefile of categorical results untrained
#f = open('ML_picklefiles/LSTMB_categorical_modelinfos.pckl', 'wb') 
